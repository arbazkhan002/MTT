...
#        string = self.__append__(tokenizer.normalize(self.token.token), self.token.pos_tag)
        ...
        string = self.__append__(string, self.token.isInsideVic)
#        string = self.__append__(string, self.token.timezone)
...
def add_output(tokens, chunks, token_idx, chunk_idx, anno, index, label, output, prep, geo, is_sen_case_sensitive, g, v):
    ...
    if anno:
        temp.identifiability = "B-" + str(anno.identifiability) if index == 0 else "I-" + str(anno.identifiability)
        temp.gran_level = "B-" + str(anno.gran_level) if index == 0 else "I-" + str(anno.gran_level)
        temp.normalised = "B-" + str(anno.normalised) if index == 0 else "I-" + str(anno.normalised)
        temp.vernacular = "B-" + str(anno.vernacular) if index == 0 else "I-" + str(anno.vernacular)
        temp.notes = anno.notes
        temp.anno_total_len = len(anno.tokens)
        temp.anno_tag = anno.tag
...
#                             print t.token + " ",
#                         print ""
                        print "here: ",token.token,annotations[index1].tokens[index2].token,annotations[index1].original
                    index2 = 0    
                    if index1 < len(annotations)-1:
						index1 += 1
                    else:
						break	
...
#                     pass
                if index1 > len(annotations)-1:
					continue
                if index2 > len(annotations[index1].tokens)-1:
					continue
                    ...
                    if index2 == 0:
                        if annotations[index1].tag != "IRREL":
                            if token_idx == 0:
                                chunk_start_count += 1
#                            else:
#                                print annotations[index1].get_placename()
                        if g != "O":
                            if annotations[index1].tag != "IRREL":
                                if g in geo_en:
                                    geo_en[g] += 1
                            ...
                            g = "B-" + g
                            print sentence
                            print annotations[index1].get_placename().lower() + "\t" + geonames[annotations[index1].get_placename().lower()].featureclass
                            geonames_count += 1
                        if v != "O":
                            if annotations[index1].tag != "IRREL":
                                if v in vic_en:
                                ...
                                if token.fcl_IOB_vic == "B-" + v:
                                    ...
                                    if v in vic_en_m:
                                        ...
                                        vic_en_m[v] = 1
                            v = "B-" + v
#                            print annotations[index1].get_placename().lower() + "\t" + vicnames[annotations[index1].get_placename().lower()].featurecode
                            vicnames_count += 1
                        if g != "O" and v != "O":
                            geovic_count += 1
...
#                    output.append(temp)
    ...
    print "geovic_count: " + str(geovic_count)
    print "chunk_start_count: " + str(chunk_start_count)
    
    sorted_geo_en = sorted(geo_en.iteritems(), key = operator.itemgetter(1))
    print "sorted_geo_en: " + str(sorted_geo_en)
    sorted_geo_en_m = sorted(geo_en_m.iteritems(), key = operator.itemgetter(1))
    print "sorted_geo_en_m: " + str(sorted_geo_en_m)
    sorted_vic_en = sorted(vic_en.iteritems(), key = operator.itemgetter(1))
    print "sorted_vic_en: " + str(sorted_vic_en)
    sorted_vic_en_m = sorted(vic_en_m.iteritems(), key = operator.itemgetter(1))
    print "sorted_vic_en_m: " + str(sorted_vic_en_m)
    print "geo_match_count: " + str(geo_match_count)
    print "vic_match_count: " + str(vic_match_count)
...
def main():
    ...
    for anno in result:
        if anno.tag == "IRREL":
            continue
        if anno.tokens[0].pos_tag not in first_word_pos:
            first_word_pos[anno.tokens[0].pos_tag] = 1
        else:
            first_word_pos[anno.tokens[0].pos_tag] += 1
        if anno.is_all_nnps():
            all_nnps_count += 1
        (key, value) = anno.get_most_freq_pos()
        if key not in most_freq_pos:
            most_freq_pos[key] = 1
        else:
            most_freq_pos[key] += 1
        anno_count += 1
    print "anno_count: " + str(anno_count)
    sorted_first_word_pos = sorted(first_word_pos.iteritems(), key = operator.itemgetter(1))
    print "first_word_pos: " + str(sorted_first_word_pos)
    print "sum: " + str(sum(first_word_pos.values()))
    print "all_nnps_count: " + str(all_nnps_count)
    sorted_most_freq_pos = sorted(most_freq_pos.iteritems(), key = operator.itemgetter(1))
    print "most_freq_pos: " + str(sorted_most_freq_pos)
    print "all_most_freq_pos: " + str(sum(most_freq_pos.values()))
