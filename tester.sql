...
BEGIN
	...
	ELSIF st_intersects(line,xs[num]) then	
	...
	--~  Now postgis cant detect an intersection point to be located on it
	--~  although the bounding boxes of line and the point intersect
	...
	ELSE
		...
		IF st_geometrytype(rowg)!='ST_Point' THEN
			res := splitN(rowg, xs[num:array_upper(xs,1)]);
			FOR i in 1..array_upper(res,1) LOOP
				IF NOT st_isempty(res[i]) THEN
					geom := array_prepend(res[i],geom);
				END IF;	
			END LOOP;
		END IF;
		rowg := st_line_substring(line,temp,1) ;
		IF st_geometrytype(rowg)!='ST_Point' THEN
			res := splitN(rowg, xs[num:array_upper(xs,1)]);
			FOR i in 1..array_upper(res,1) LOOP
				IF NOT st_isempty(res[i]) THEN
...
BEGIN
	DELETE FROM dummy;
	FOR row2 in EXECUTE 'select distinct geom from split_points' LOOP
